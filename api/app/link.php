	...
	if( $branch != 'master' )
	{
		...
		switch( $result['service_type'] )
		{
			case 'mysql':
				...
				$link->query("CREATE DATABASE `{$new_service}` CHARACTER SET utf8 COLLATE utf8_unicode_ci");
				$link->query("GRANT USAGE ON * . * TO '{$result['service_name']}'@'%' WITH MAX_QUERIES_PER_HOUR 0 MAX_CONNECTIONS_PER_HOUR 0 MAX_UPDATES_PER_HOUR 0 MAX_USER_CONNECTIONS 0");
				$link->query("GRANT ALL PRIVILEGES ON `{$new_service}` . * TO '{$result['service_name']}'@'%'");
